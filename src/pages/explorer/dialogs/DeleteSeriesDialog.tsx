...
export function DeleteSeriesDialog({
  ...
  const mutation = useExplorerMutation({
    name: 'Delete series',
    fn: () => seriesService.delete(seriesId),
    onSuccess: () => { onClose(); onSuccess() },
    successMessage: `"${seriesName}" deleted`,
  })
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[440px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Permanently delete <strong>"{seriesName}"</strong>. This cannot be undone.
          </DialogDescription>
        </DialogHeader>
          ...
          {loadingImpact ? (
            ...
            <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Remove all modules and documents from this series first.
              </p>
            </div>
          ) : impact ? (
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              No dependencies found. Series can be safely deleted.
            </div>
          ) : null}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {mutation.loading ? (
              ...
              <><Trash2 className="w-3.5 h-3.5 mr-1.5" /> Delete series</>
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
