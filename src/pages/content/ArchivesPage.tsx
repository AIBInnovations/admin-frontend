...
import { Archive, RotateCcw, Trash2, Video, FileText, Image, MoreVertical } from 'lucide-react'
import {
...
export function ArchivesPage() {
  ...
  const handleRestore = async (item: ArchivedItem) => {
    try {
      ...
      if (response.success) {
        toast.success(`${item._type.charAt(0).toUpperCase() + item._type.slice(1)} restored successfully`)
        fetchItems()
      } else {
  ...
  const columns: ColumnDef<ArchivedItem>[] = [
    ...
    },
    {
      ...
      width: 'w-28',
      cell: (item) => (
        <Badge className={`text-[10px] capitalize ${typeBadgeColors[item._type] || ''}`}>
          {item._type}
        </Badge>
      ),
    },
